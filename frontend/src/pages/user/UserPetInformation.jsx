...
function UserPetInformation() {
    ...
    const [vaxPageLimit, setVaxPageLimit] = useState(4);
    ...
    const loadPetVaccinations = async () => {
      let vaccs;
      await axios.get(`http://localhost:5001/api/vaccinations/pet?petid=${id}`)
      .then(response => vaccs = response.data.data)
      .catch(err => console.error(err))
      return vaccs;
    }
    ...
    const deletePet = async () => {
      let sessionToken = sessionStorage.getItem('jwt-token');
      await axios.delete(`http://localhost:5001/api/pets/remove/${id}`, 
        {
          headers: {
            'Authorization': `Bearer ${sessionToken}`
          }
    ...
    useEffect(() => {
      ...
      vaccPromise.then(vaccs => {
        let vaxArr = [];
        vaccs.forEach((vax, index) => {
          if(index < vaxPageLimit){
            let vaxObj = {
              "number": index+1,
              "vaccine_name": vax.vaccine_name,
              "date": convertDate(vax.DATE),
              "time": vax.TIME
            }
            vaxArr.push(vaxObj);
          }
        })
        setVaxData(va => va = vaxArr)
      })
    },[])
    return (
      <>
            ...
            <section className="py-4 lg:py-6 flex relative items-center justify-center">
              <section className="flex flex-col gap-2 items-center ">
                <section className="relative w-[100px] h-[100px] md:w-[120px] md:h-[120px] lg:w-[150px] lg:h-[150px]">
                    <img src={`/pet/${petInfo.image}`} alt="" className=" w-full h-full object-cover bg-azure rounded-full" />
                    <section className="absolute bottom-1 right-1 bg-silver rounded-full h-[30px] w-[30px] md:h-[38px] md:w-[38px] flex items-center justify-center cursor-pointer">
                      ...
                      />
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className="fill-white-smoke h-[20px] w-[20px] md:h-[22px] md:w-[22px]" onClick={handleImgBtnClick}>
              ...
              <section className="absolute top-10 right-96 flex flex-col gap-2">
                ...
                <section className="flex items-center cursor-pointer">
                  ...
                  <p className="font-lato text-content-md hover:underline text-fire-engine-red" onClick={deletePet}>Delete Pet</p>
                </section>
              </section>
            </section>
          )}
