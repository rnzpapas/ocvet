...
function Button({style, txtContent, isActive = true, onClickFunc}) {
  ...
  return (
    <button 
      className={`font-lato py-1 px-2 md:px-10 rounded-[5px] uppercase text-content-xtrasm xxl:text-content-sm xxxl:text-content-md font-semibold hover:bg-raisin-black-light ${isActive ? activeStyle : inactiveStyle } ${style}`}
      onClick={onClickFunc}
    > 
      {txtContent} 
    </button>
