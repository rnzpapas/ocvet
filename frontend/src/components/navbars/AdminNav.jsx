    ...
    return (
        <nav className="w-[280px] h-screen bg-linen flex flex-col items-center">
            ...
            <section className="w-full h-[60%] flex flex-col gap-3">
                {navLinks.map((navLink, index) => (
                    <Link to={navLink.targetElement} key={index}>
                        <section id={index} className="w-full flex justify-end px-5 py-3 hover:bg-raisin-black-light/25 cursor-pointer"> 
            ...
            <section className="h-[10%] flex gap-2 items-center justify-center cursor-pointer" onClick={logout}>
                <p className="font-instrument-sans uppercase font-semibold text-white-smoke">logout</p>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className="w-[15px] h-[15px] fill-white-smoke">
