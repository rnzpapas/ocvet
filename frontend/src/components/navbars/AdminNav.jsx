                ...
                {navLinks.map((navLink, index) => (
                    <Link to={navLink.targetElement} key={index}>
                        <section id={index} className="w-full flex justify-end px-5 py-3 group hover:bg-raisin-black-light  cursor-pointer"> 
                                <h5 className=" font-instrument-sans text-headline-sm font-semibold text-raisin-black group-hover:text-white-smoke uppercase cursor-pointer">{navLink.txtContent}</h5>
                        </section>
                    </Link>
                ))}
            </section>
            <section className="h-[10%] flex gap-2 items-center justify-center cursor-pointer" onClick={logout}>
                <p className="font-instrument-sans uppercase font-semibold text-raisin-black text-headline-mbl">logout</p>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className="w-[12px] h-[12px] fill-raisin-black">
